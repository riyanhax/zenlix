        ...
        if (isset($_SESSION['hd.rustem_sort_in'])) {
            switch ($_SESSION['hd.rustem_sort_in']) {
                ...
                case 'lock':
                    ...
                    break;
                case 'personal':
                    $r = lang('CONF_EMAIL_GATE_filter') . ": " . "<span class=\"label label-default\">" . lang('ticket_sort_personal') . "</span>";
                    break;
...
function getLastComment($tid)
{
		 global $dbConnection;
		$stmt = $dbConnection->prepare('SELECT comment_text from comments where t_id=:tid order by id DESC');
        $stmt->execute(array(':tid' => $tid));
		$res1 = $stmt->fetchAll();

		if ($res1) {
            return $res1[0]['comment_text'];
        }
}
...
function get_unit_stat_create($in, $start = null, $stop = null) {
    global $dbConnection;
...
function get_unit_stat_free($in, $start = null, $stop = null) {
    global $dbConnection;
...
function get_unit_stat_lock($in, $start = null, $stop = null) {
    global $dbConnection;
...
function get_unit_stat_ok($in, $start = null, $stop = null) {
    global $dbConnection;
...
function get_users_from_units_by_user_api($idd) {
    global $dbConnection;
...
/** FatFreeFramework function created by arava (newbie.jedicoder@gmail.com)
 * @param array $var
 * @param $col
 * @return array
 */
function f3pick(array $var,$col) {
    return array_map(
