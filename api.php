            ...
            if (isset($data_json->uniq_id)) {
                ...
                if (validate_user_by_api($data_json->uniq_id)) {
                    ...
                    if (get_conf_param('sla_system') == "true") {
                        ...
                        $stmt = $dbConnection->prepare('SELECT id, name FROM sabj_pl');
                        $stmt->execute();
                        $dep = $stmt->fetchAll();
...
values (:comment, :n, :user_comment, :tid_comment)');
            ...
            if (isset($data_json->uniq_id, $data_json->user_to_id, $data_json->subj, $data_json->sabj_pl, $data_json->msg, $data_json->client_id, $data_json->unit_id, $data_json->prio)) {
                ...
                if (validate_user_by_api($data_json->uniq_id)) {
                    ...
                    */
                    ...
                    if ($error_code == false) {
						...
						$sabj_pl_id = $data_json->sabj_pl;
                        $stmt_sabj_pl = $dbConnection->prepare('SELECT name from sabj_pl where id=:p_id');
                        $stmt_sabj_pl->execute(array(
                            ':p_id' => $sabj_pl_id
                           ));
                        $row_sabj_pl = $stmt_sabj_pl->fetch(PDO::FETCH_ASSOC);
                        $sabj_pl = $row_sabj_pl['name'];
