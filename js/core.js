...
*/
    ...
    function makemytime(s) {
		...
		$('time#g').each(function(i, e) {
            var time = moment($(e).attr('datetime'));
            $(e).html('<span>' + time.format("mm") + '</span>');
        });
		$('time#k').each(function(i, e) {
        ...
        */
        ...
        $('body').on('click', 'button#add_scheduler', function(event) {
            ...
            $.ajax({
                ...
                url: ACTIONPATH,
                dataType: "json",
                data: "mode=add_cron" + "&client_id_param=" + $("#client_id_param").val() + "&to=" + $("#to").val() + "&s2id_users_do=" + $("#users_do").val() + "&prio=" + $("#prio").val() + "&subj=" + $("#subj").val() + "&sabj_pl=" + $("#sabj_pl").val() + "&msg=" + $("#msg").val() + "&period=" + $("#period").val() + "&day_field=" + $("#day_field").val() + "&week_select=" + $("#week_select").val() + "&month_select=" + $("#month_select").val() + "&time_action=" + $("#time_action").val() + "&action_start=" + $("#action_start").val() + "&action_stop=" + $("#action_stop").val() + "&status_action=" + $("#status_action").val(),
                success: function(html) {
                    //console.log(html);
                    $.each(html, function(i, item) {
...
*/
                                ...
                                $.each(html, function(i, item) {
                                    ...
                                    if (item.res == false) {
                                        ...
                                        if (item.priv == false) { //console.log('not add');
                                            ...
                                            $("#status_action").val('');
                                            console.log('status action null core.js 1427');
                                            $("#fio").val('');
                                            makemytime(true);
                                        }
                                    }
    ...
    if (ispath('events_report')) {
        ...
        */
        $('body').on('click','button#eventsreport',function(event) {
            event.preventDefault();
            var uid        = $('#user').val();
            ...
            $.ajax({
                type: 'POST',
                    url: ACTIONPATH,
                data: "mode=getEventsReport"+"&uid="+uid+"&department="+department+"&start="+start_time+"&end="+stop_time,
        ...
        $('body').on('click', 'button#events_sort', function(event) {
            event.preventDefault();
            //var pt = $("#page_type").attr('value'); // filter type, IN or OUT
            var column = $(this).attr('value'); // button type, like personal or free
            $.ajax({
                type: "POST",
                url: ACTIONPATH, // url/action
                data: "mode=events_sort" + "&column=" + encodeURIComponent(column),
                success: function(resp) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: ACTIONPATH,
                        data: "mode=getEventsReport"+"&uid="+uid+"&department="+department+"&start="+start_time+"&end="+stop_time+"&sort="+resp.sort+"&col="+resp.col,
                        success: function(response) {
                            $('#report_response').html(response);
                            //$(".knob").knob();
                            //makemytime();
        ...
        $('body').on('change','#department',function(event) {
            ...
            if (dep) {
                ...
                $.ajax({
                    type: 'POST',
                    url: ACTIONPATH,
                    data: "mode=get_users_by_dep&department="+dep,
                    success: function(response) {
                        response = JSON.parse(response);
                        if (response) {
                            var option = "<option value='-1'>Не выбрано</option><option value='0'>Все</option>";
                            for (var i = 0; i < response.length; i++) {
                                option += "<option value='"+response[i].id+"'>"+response[i].login+"</option>";
                            }
                        }
...
//startChatWithClient
    ...
    if (ispath('ticket')) {
        /**
         *  Ticket cancel in the ticket list and inside current ticket
         */
        $('body').on('click','button#action_list_cancel',function(event) {
            //return confirm('Уверены что хотите удалить тикет?');
            //console.log('arava');
            event.preventDefault();
            var status_ll = $(this).attr('status');
            var tr_id = $(this).attr('value');
                //var elem = '#tr_' + tr_id; // ticket row identifier
                //var elb = '.ela_' + tr_id;
            var us = $(this).attr('user');
            if (status_ll === 'cancel') {
                //console.log("mode=ticket_cancel"+"&tid="+tr_id+"&user="+encodeURIComponent(us));
                //return;
                $.ajax({
                    type: "POST",
                    url: ACTIONPATH,
                    data: "mode=ticket_cancel"+"&tid="+tr_id+"&user="+encodeURIComponent(us),
                    success: function(response) {
                        response = JSON.parse(response);
                        if (response.status === 'ok') {
                            if (document.getElementById('tr_'+ tr_id)) {
                                document.getElementById('tr_'+ tr_id).style.display = 'none';
                            }
                            window.location.href = '/list?in';
                        } else {
...
*/
    ...
    if (ispath('create')) {
        ...
        //select_init_user
        //$('body').on('click', 'a#select_init_user', function(event) {
        $('body').on('change', '#to'/*'a#select_init_user'*/, function(event) {
            //$('#select_init_user').trigger('click');
            event.preventDefault();
            console.log($(this).attr('param-hash'));
            var ulogin = $('#user_name_login').val();
            var uinitid = $('#user_init_id').val();
        ...
        $('body').on('click', 'button#enter_ticket', function(event) {
            ...
            if (check_form_ticket() == 0) {
                console.log('check form ticket == 0 core.js 2914');
                console.log('enter ticket core.js 2915');
                enter_ticket();
            }
        ...
        function check_form_ticket() {
            ...
            */
			...
			//if ((p == null) || (p == "0")) {
                //error_code = 1;
                //$("#for_subj").popover('show');
                //$("#for_sabj_pl").addClass('has-error');
                //setTimeout(function() {
                    //$("#for_sabj_pl").popover('hide');
                //}, 2000);
            //}
            if (m == 0) {
                error_code = 1;
...
console.log(to);
        ...
        function enter_ticket_client() {
            ...
            $.ajax({
                ...
                //async: false,
                url: ACTIONPATH,
                data: "mode=add_ticket" + "&type_add=client" + "&user_init_id=" + encodeURIComponent($("#user_init_id").val()) + "&user_do=" + encodeURIComponent(u_do) + "&subj=" + encodeURIComponent($("#subj").val()) + "&sabj_pl=" + encodeURIComponent($("#sabj_pl").val()) + "&corpus_id=" + encodeURIComponent($("#corpus-id").val()) + "&room_numb=" + encodeURIComponent($("#room-numb").val()) + "&msg=" + encodeURIComponent($("#msg").val()) + "&unit_id=" + encodeURIComponent($("#to").val()) + "&prio=" + encodeURIComponent($("#prio").val()) + "&hashname=" + encodeURIComponent($("#hashname").val()) + "&deadline_time=" + deadline_time + "&" + add_from,
                success: function(html) {
                    //console.log(html);
        ...
        /**
         * Ticket creating (udpated by arava 28.02.2019 19:36 newbie.jedicoder@gmail.com)
         */
    ...
    */
    ...
    if (ispath('list')) {
        ...
        $('body').on('click', 'button#sort_list', function(event) {
            event.preventDefault();
            var pt = $("#page_type").attr('value'); // filter type, IN or OUT
            var st = $(this).attr('value'); // button type, like personal or free
            //console.log("mode=sort_list" + "&pt=" + encodeURIComponent(pt) + "&st=" + encodeURIComponent(st));
            //return;
...
*/
    getDepartmentTypes = function getDepartmentTypes(dname)
    {
        $.ajax({
            type: "POST",
            url: ACTIONPATH, // url/action
            data: 'mode=get_ticket_type' + '&dname=' + encodeURIComponent(dname),
            success: function(response) {
                response = JSON.parse(response);
                // set into select types
                var type   = document.getElementById('subj');
                var count  = response.length;
                var option = "<option></option>";

                for (var i = 0; i < count; i++) {
                    if (count === 1) {
                        option += "<option value="+response[i]+" selected>"+response[i]+"</option>";
                    } else {
