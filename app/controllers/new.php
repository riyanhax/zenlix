...
if (validate_user($_SESSION['helpdesk_user_id'], $_SESSION['code'])) {
    if ($_SESSION['helpdesk_user_id']) {
        ...
        {
            /**
             * Get departments during ticket creating
             * @return array
             */
			...
			public function get_sabj_pl_list() {
                global $dbConnection;
                $res = array();
                $stmt = $dbConnection->prepare('SELECT name FROM sabj_pl order by sort_id ASC');
                $stmt->execute();
                $res1 = $stmt->fetchAll();
                foreach ($res1 as $row) {
                    array_push($res, array(
		...
		//$subj_list = $new_ticket_form->get_subj_list(); // this column handler by ajax
		$sabj_pl_list = $new_ticket_form->get_sabj_pl_list();
        ...
        try {
            ...
            echo $template->render(array(
                ...
                //'CREATE_TICKET_ME' => $_SESSION['helpdesk_user_login'],//lang('CREATE_TICKET_ME') ,
				...
				'NEW_sabj_pl' => lang('NEW_sabj_pl') ,
                'NEW_subj_msg' => lang('NEW_subj_msg') ,
                'subj_list' => '',//$subj_list,
				'sabj_pl_list' => $sabj_pl_list,
                ...
                'user_name_login' => $_SESSION['helpdesk_user_login'],//get_user_val('login') ,
